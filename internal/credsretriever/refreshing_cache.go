...
const (
	defaultActiveRequestWaitTime = 100 * time.Millisecond
...
func (r *cachedCredentialRetriever) GetIamCredentials(ctx context.Context,
	...
	for {
		...
		if errActiveRequest, ok := r.internalActiveRequestCache.Get(request.ServiceAccountToken); ok {
			...
			waitTime := defaultActiveRequestWaitTime * (1 << uint(n)) // 2^n backoff
			if waitTime > maxActiveRequestWaitTime {
				break
			}

			log.Infof("Retrying waiting for internalActiveRequestCache in %v\n", waitTime)
			time.Sleep(waitTime)
			n++
		} else {
